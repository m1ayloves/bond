<script setup>
import {getContactInfo, updateContact} from '@/api/contacts-api.js'
...
async function show(id) {
  ...
  // 处理 tag 为 null 或 undefined 的情况
  const tag = data.tag || ''
  selectedTagList.value = tag ? tag.split(',') : [] // 确保始终是一个有效数组
  form.value = data
  open.value = true
}
...
async function handleSubmit() {
  try {
    ...
    if (selectedTagList.value.length === 0) {
      form.value.tag = null // 或者设置为其他默认值，比如 'Default'
    } else {
      form.value.tag = selectedTagList.value.join(',').replace(/^,/, '') // 移除可能的前置逗号
    }
